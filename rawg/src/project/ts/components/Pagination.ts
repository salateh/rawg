...
function get_a_Page(respPagination: string | null) {
  const urlString: string = `${respPagination}`;

  const queryString: string = urlString.split("?")[1] || "";

  // Создаем объект URL
  const urlPag = new URLSearchParams(queryString);

  const PAGE: string | null = urlPag.get("page") ? urlPag.get("page") : "1";

  return PAGE;
}

export function createPagination(): HTMLElement | null {
