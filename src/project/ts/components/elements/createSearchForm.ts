...
export function createSearchForm(): HTMLElement {
  ...
  const handleSuggestions = (params: string) => {
    suggestionsSearchGames(params);
  };
  const debouncedSugg = debounce(handleSuggestions, 300);
  form.addEventListener("input", () => {
    const query = input.value.trim();
    if (query) (suggestions.replaceChildren(), debouncedSugg(query));
